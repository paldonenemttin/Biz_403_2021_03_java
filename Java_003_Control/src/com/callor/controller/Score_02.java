...
public class Score_02 {
	...
	public static void main(String[] args) {
		...
		// intNum1에 저장된 정수를
		//"실수형으로 변환"하여 오차를 무시하고 저장하라
		
		floatNum1 = intNum1;
		
		// 실수값(float)를 정수형변수 저장하라 라고 명령을 내리면
		//자바 컴파일러는
		// 이 코드를 실행하면 실수값의 소수점 이하가
		// 잘릴것이다라는 것을 안다
		// 또한 이 코드는 개발자의 실수라고 판단하고 문법오류로 보여준다
		
		// 개바ㅣㄹ자가 실제 값의 소수점 이하가 잘리는 것을
		// 알고 있으며
		// 소수점이하 값이 필요 없어서
		// 정수형 변수에 저장하고 싶다
		 ...
		 * 실수값, 실수형 변수에 저장된 값을
		 * 정수형변수에 저장하려고 시도하면
		 * 컴파일러는 문제가 일어날 수 있다는 것을 인식하고 오류를 보여준다
		 * 
		 * 만약 개발자가 오차를 무시하고
		 * 강제로 실수값을 정수형에 변환하여 저장하고 싶음
		 * 강제로 변환하는 코드를 삽입
		 * 실수값 실수형변수에 저장된 값에서
	     * 소수점이 값을 무조건 잘라버리고
		 * 정수부분만 변수에 저장한다
