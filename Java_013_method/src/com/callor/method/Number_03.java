...
public class Number_03 {
	...
	public static void main(String[] args) {
		...
		System.out.println("QUIT는 종료");
		System.out.println(">> ");
		String strInput = scan.nextLine();
		
		// trim() : 문자열의 앞, 뒤엥 붙은  white space 를 제거하는 method
		
		strInput = strInput.trim();
		strInput.equals("QUIT");
		
		// 입력된 문자열에 혹시 있을지 모르는 white space를
		...
		if(strInput.trim().equals("QUIT")) {
			 ...
			 * 코드를 중단하지 않고, 사용자에게 메시지들을 전달하고
			 * 적절한 연산코드를 추가하여 코드를 계속 진행하도록 하는 조치
			 * 
			 * 예외처리 코드는 개발자에게는 상당히 불필요한 코드를 추가하여
			 * 부담을 주는 것일 수도 있다
			 * 하지만, 적절한 예외처리를 해준 코드는 
			 * EndUser(실제사용자)에게 불편함을 최소한으로 주는 적절한 방법이다
			 * 
			...
			try {
				...
				//e.printStackTrace();
				System.out.println("숫자가 아닌 값을 입력함");
