...
public class Number_02 {
	...
	public static void main(String[] args) {
		...
		/*
		 * 데이터를 단독ㅇ로 처리할때는 큰 문제가 없는데
		 * 파일에 저장, 파일에서 일긱
		 * 네트워크(인터넷)등을 통하여 전송(공유)등을 하여
		 * 데이터가 컴퓨터와 컴퓨터간의 이동할때
		 * 데이터를 효율적이고 오류가 나ㅣㅈ 않도록
		 * 이동하는 다양ㅎ나 방법이 있다
		 * 여러가지 방법이 취하는 많은 공통된 부분에
		 * 모든 데이터를 문자열화하여 전송하는 방법이 있다
		 * 숫자, 문자, 특수기호 등을모두 문자열(ASCII)로
		 * 전송하고, 받는 곳에서 사용하기 전에 적절한
		 * type으로 변경하여 사용한다
		 * 
		 * Java에서는 WrapperClass.parse*() method나
		 * WrapperClass.ValueOf() method를 사용하여
		 * 문자열형 숫자를 숫자값으로 변환시키는 것을 지웒나다
		 * 
		 * 이과정에서 숫자값으로 변환할 수 없는 문자열이
